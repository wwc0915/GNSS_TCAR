...
/*该文件处理包含GPS、BDS2、Galileo系统的载波相位差分定位*/
...
void ProGRCTrans(CMatrix& TT_G, CMatrix& TT_C, CMatrix& TT_E, CMatrix& TT_ALL)
{
	int Row_G = TT_G.Row;
	int Col_G = TT_G.Col;
	int Row_C = TT_C.Row;
	...
	int Tot_Row = Row_G + Row_C + Row_E;
	int Tot_Col = Col_G + Col_C + Col_E;
	...
	for (i = 0; i < Row_E; i++)
		for (j = 0; j < Col_E; j++)
			TT_ALL[Row_G + Row_C + i][Col_G + Col_C + j] = TT_E[i][j];
}

//重载CheckNewSats函数，检测新卫星
void CheckNewSats(int* SatNum, int* oldSatNum, int SatPRN[3][25], int oldSatPRN[3][25], int NewSatPRN[3][25], int* NewSatNum)
{
	for (int i = 0; i < 3; i++)
	{
		int newsatsnum = 0;
...
void UpdataState(CMatrix& QX, CMatrix& X, int* SatNum, int* oldSatNum, int SatPRN[3][25], int oldSatPRN[3][25], int* RefSat, int* OldRef)
{
	CMatrix T[3], ALL_T, Matrix_t;
	...
	ALL_NUM = SatNum[0] + SatNum[1] + SatNum[2] - sys;
	oldALL_NUM = oldSatNum[0] + oldSatNum[1] + oldSatNum[2] - oldsys;
	...
	ProGRCTrans(T[0], T[1], T[2], ALL_T);//拼接矩阵
...
bool trisys_RTK(string filename1, string filename2)
{
	//默认第一个站为基准站，第二个为流动站
	...
	//输出差分定位结果用
	string name1 = filename2.substr(0, 3) + "_trisys_RTK.txt";
	const char* output_filename1 = name1.c_str();
	FILE* fp3;
	fopen_s(&fp3, output_filename1, "w");

	int OldSatPRN[3][25] = { 0 };
	int OldRefPRN[3] = { -1,-1,-1};
	int OldSatNum[3] = { 0 };
	...
	for (int i = 0; i < sppfile2.liyuan_num; i++)
	{
		...
		//分离GPS、BDS-2、Galileo卫星，便于后续单系统、多系统处理
		Obs_epoch GPSdata;
		Obs_epoch BDS2data;
		Obs_epoch Galileodata;

		int SatPRN[3][25] = { 0 };		//当前历元卫星号 依次存储
		int SatNum[3] = { 0 };	//当前历元各系统卫星数目
		int RefPRN[3] = { 0 };  //当前历元各系统参考卫星号

		int NewSatNum[3] = { 0 };
		int NewSatPRN[3][25] = { 0 };

		int k = 0;
		int m = 0;
		int gpsnum, bds2num, galileonum;
		gpsnum = bds2num = galileonum = 0;
		for (m = 0; m < tempData.sat_num; m++)
		{
			...
			if (tempData.sat1[m].sattype == "E")
			{
				...
				SatPRN[2][galileonum] = tempData.sat1[m].numofsat;
				galileonum++;
			}
		}
		...
		//卫星数至少要4颗
		if ((GPSdata.sat_num + BDS2data.sat_num + Galileodata.sat_num) < 4)
		{
			printf("卫星数少于4颗\n");
			bInit = false;
			continue;
		...
		int num_x_p = (numgps_x + numbds2_x + numgal_x) * 2;

		//待估参数个数：ΔX，ΔY，ΔZ，每颗卫星的模糊度
		int num_x = numgps_x + numbds2_x + numgal_x + 3;

		//模糊度个数
		int num_n = numgps_x + numbds2_x + numgal_x;
		...
		for (int k1 = 0; k1 < numgal_x; k1++)
			...
			{
				R[k1 + numgps_x + numbds2_x ][k2 + numgps_x + numbds2_x ] = RE_P[k1][k2];
			}
		//载波
		for (int k1 = 0; k1 < numgps_x; k1++)
			for (int k2 = 0; k2 < numgps_x; k2++)
			{
				R[k1 + numgps_x + numbds2_x + numgal_x][k2 + numgps_x + numbds2_x + numgal_x] = RG_L[k1][k2];
			}

		for (int k1 = 0; k1 < numbds2_x; k1++)
			for (int k2 = 0; k2 < numbds2_x; k2++)
			{
				R[k1 + numgps_x * 2 + numbds2_x + numgal_x][k2 + numgps_x * 2 + numbds2_x + numgal_x] = RC2_L[k1][k2];
			}

		for (int k1 = 0; k1 < numgal_x; k1++)
			for (int k2 = 0; k2 < numgal_x; k2++)
			{
				R[k1 + numgps_x * 2 + numbds2_x * 2 + numgal_x][k2 + numgps_x * 2 + numbds2_x * 2 + numgal_x] = RE_L[k1][k2];
			}
		...
		if (GPSdata.sat_num >= 4 && BDS2data.sat_num >= 4)
		{
			...
			fprintf(fp3, "EpNum: %2d, %2d, %2d, %10.6f, %2d, %2d, %2d, %15.8g,%15.8g, %15.8g, %15.8g\n", i + 1, tempData2.hour, tempData2.minute, tempData2.second,
				GPSdata.sat_num, BDS2data.sat_num, Galileodata.sat_num, ratio, ErrorX_NEU[0][0], ErrorX_NEU[1][0], ErrorX_NEU[2][0]);
		}
